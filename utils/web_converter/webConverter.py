...
if args.repopath is not None:
    parent = Path(args.repopath).expanduser().resolve()
...
try:
    ...
    print("\n[!] Requirements are not satisfied. Install anglerfish: python3 -m pip install anglerfish\n")
    exit(1)

license_file_path = str(os.path.join(str(parent), "LICENSE"))
q = PurePath('esp8266_deauther')
deauther_dir = parent / q
arduino_file_path = str(os.path.join(str(deauther_dir), "webfiles.h"))
dir = deauther_dir / PurePath('web_interface')
q = PurePath('data')
datadir = deauther_dir / q
