...
void CLI::update() {
    ...
    else if ((queue->size() > 0) && !delayed && !scan.isScanning() && !attack.isRunning() && !hsCaptureActive()) {
...
void CLI::runCommand(String input) {
    ...
    // set ssid listsize <size>
    else if ((list->size() == 4) && eqlsCMD(0, CLI_SET) && eqlsCMD(1, CLI_SSID) && eqlsCMD(2, CLI_LISTSIZE)) {
        ssids.setListSize((uint16_t)list->get(3).toInt());
        ssids.save(true);
        prnt("SSID list size = ");
        prntln(ssids.getListSize());
    }
    ...
    // load [<type>] [<file>]
        ...
        if ((list->size() == 1) || eqlsCMD(1, CLI_ALL)) {
            ...
            load ? names.load() : names.saveNamesIfNeeded();
            load ? settings::load() : settings::save(false);

            if (!load) scan.saveScanIfNeeded();
            return;
        }
        ...
        if (list->size() == 3) { // Todo: check if -f or filename
            ...
            else if (eqlsCMD(1, CLI_NAME)) load ? names.load() : names.saveNamesIfNeeded();
            else if (eqlsCMD(1, CLI_SETTING)) {
                load ? settings::load() : settings::save(true);
                if (!load) wifi::syncCaptivePortal();
            }
            else if ((eqlsCMD(1, CLI_SCAN) || eqlsCMD(1, CLI_AP) || eqlsCMD(1, CLI_STATION)) && !load) scan.saveScanIfNeeded();
            else parameterError(list->get(1));
        }
    }
    ...
    // set <setting> <value>
        ...
        // Work mode
        else if (eqls(str, S_JSON_WORKMODE)) newSettings.workmode = (uint8_t)unsignedVal;
    ...
    // ===== CAPTURE HANDSHAKE (experimental) ===== //
    // capture handshake -ap <id> [-st <id>] [-t <time>]
    // capture handshake stop | status
    else if (eqlsCMD(0, CLI_CAPTURE) && (list->size() >= 2) && eqlsCMD(1, CLI_HANDSHAKE)) {
        if ((list->size() >= 3) && eqlsCMD(2, CLI_STOP)) {
            hsCaptureStop(true);
            return;
        }

        if ((list->size() >= 3) && eqlsCMD(2, CLI_STATUS)) {
            prntln(hsGetStatusJson());
            return;
        }
        ...
        for (int i = 2; i < list->size(); i++) {
            if (eqlsCMD(i, CLI_AP)) {
                i++;
                apId = list->get(i).toInt();
            } else if (eqlsCMD(i, CLI_STATION)) {
                i++;
                String stArg = list->get(i);

                if (stArg.indexOf(':') >= 0) {
                    strToMac(stArg, stMacBuf);
                    stMacOk = macValid(stMacBuf);
                    stId    = -1;
                } else {
                    stId = stArg.toInt();
                }
        ...
        if (apId < 0) {
            prntln("HS: need -ap <id>");
            return;
        }

        if (stId >= 0 && stId >= stations.count()) {
            prnt(F("HS: station #"));
            prnt(stId);
            prntln(F(" not in list — run scan stations or use -st aa:bb:cc:dd:ee:ff"));
        }

        if (!stMacOk && stId < 0) {
            prntln(F("HS: no -st — deauth all on AP; use -st <id> for best EAPOL chance"));
        }

        if (stMacOk) hsCaptureStart(apId, stMacBuf, time);
        else hsCaptureStart(apId, stId, time);
    }
    ...
    // startap [-p <path][-s <ssid>] [-pswd <password>] [-ch <channel>] [-h] [-cp]
        ...
        wifi::startNewAP(path, ssid, password, ch, hidden);
    }
    ...
    // screen <on/off>
        ...
        prntln(F("Display not available (headless build)"));
    }
#endif /* USE_DISPLAY */
