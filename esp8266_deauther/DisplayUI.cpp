...
void DisplayUI::drawString(int, int, String) {}

void DisplayUI::drawString(int, String) {}

void DisplayUI::drawLine(int, int, int, int) {}
...
void DisplayUI::draw(bool force) {
    uint32_t interval = drawInterval;

    if (scan.isScanning() || scan.isSniffing() || attack.isRunning()) {
        interval = 250;
    }

    if (force || ((currentTime - drawTime > interval) && currentMenu)) {
