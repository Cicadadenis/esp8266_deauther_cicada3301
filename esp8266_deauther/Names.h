...
class Names {
    public:
        ...
        /** No-op when list unchanged (for UI `save names` polling). */
        void saveNamesIfNeeded();
        /** RAM cache for GET /names.json (avoids LittleFS read on poll). */
        bool getJsonCache(String& out) const;
        /** Zero-copy view; invalid when namesDirty. */
