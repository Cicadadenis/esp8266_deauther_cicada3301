...
void Stations::print(int num, bool header, bool footer) {
    ...
    prnt(leftRight(String(SPACE) + getDisplayNameStr(num), String(), 17));
...
String Stations::getDisplayNameStr(int num) {
    if (!check(num)) return String();

    String custom = getNameStr(num);

    if (custom.length() > 0) return custom;

    return getVendorStr(num);
}
