...
void Names::saveNamesIfNeeded() {
    save(false);
}

bool Names::peekJsonCache(const String*& out) const {
    if (!jsonCacheValid || jsonCache.length() == 0) return false;

    if (namesDirty) return false;

    out = &jsonCache;
    return true;
}

bool Names::getJsonCache(String& out) const {
...
bool Names::hasJsonCache() const {
    return jsonCacheValid && jsonCache.length() > 0;
}

void Names::save(bool force) {
    if (!force && !namesDirty) {
        DIAG_LOGF(DIAG_DEBUG, "NAMES", "save names skipped (clean)");
        return;
    }

#if DIAG_ENABLE
    const uint32_t saveNamesT0 = millis();
#endif

    const size_t prevCacheLen = jsonCache.length();
    String       buf          = String(OPEN_BRACKET);
    String       name;
    int          c            = count();

    if (prevCacheLen > 32) buf.reserve(prevCacheLen);

    for (int i = 0; i < c; i++) {
        name = escape(getName(i));

        buf += String(OPEN_BRACKET) + String(DOUBLEQUOTES) + getMacStr(i) + String(DOUBLEQUOTES) + String(COMMA);
        buf += String(DOUBLEQUOTES) + getVendorStr(i) + String(DOUBLEQUOTES) + String(COMMA);
        buf += String(DOUBLEQUOTES) + name + String(DOUBLEQUOTES) + String(COMMA);
        buf += String(DOUBLEQUOTES) + getBssidStr(i) + String(DOUBLEQUOTES) + String(COMMA);
        buf += String(getCh(i)) + String(COMMA);
        buf += b2s(getSelected(i)) + String(CLOSE_BRACKET);
...
#if DIAG_ENABLE
    DIAG_LOGF(DIAG_DEBUG, "NAMES", "save names %lu ms", (unsigned long)(millis() - saveNamesT0));
#endif
}
