...
void setWifiChannel(uint8_t ch, bool force) {
    // Channel 0 is invalid on ESP8266 and can break AP + packet injection.
    if ((ch > 0) && (ch < 15) && ((ch != wifi_channel) || force)) {
        wifi_channel = ch;
...
bool macLocallyAdministered(uint8_t* mac) {
    if (!mac) return false;
    return (mac[0] & 0x02) != 0;
}
...
String searchVendor(uint8_t* mac) {
    if (!mac || !macValid(mac) || macBroadcast(mac) || macMulticast(mac)) return String();

    if (macLocallyAdministered(mac)) return str(ST_DEVICE_PRIVATE);
    ...
    if (vendorName.length() == 0) return str(ST_DEVICE_UNKNOWN);
...
bool fileWriteAll(File& f, const uint8_t* data, uint32_t len) {
    if (len == 0) return true;

    uint32_t offset = 0;

    while (offset < len) {
        const size_t written = f.write(data + offset, len - offset);
...
bool writeFile(String path, String& buf) {
    ...
    const bool     ok  = fileWriteAll(f, reinterpret_cast<const uint8_t*>(buf.c_str()), len);
