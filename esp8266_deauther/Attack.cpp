...
namespace {
static uint8_t beaconSendBuf[128];
}
...
bool Attack::sendBeacon(uint8_t* mac, const char* ssid, uint8_t ch, bool wpa2) {
    ...
    // =====
    const uint16_t tmpPacketSize = (uint16_t)((packetSize - 32) + ssidLen);

    if (tmpPacketSize > sizeof(beaconSendBuf)) return false;

    uint8_t* tmpPacket = beaconSendBuf;

    memcpy(&tmpPacket[0], &beaconPacket[0], 38 + ssidLen);               // copy first half of packet into buffer
    tmpPacket[37] = (uint8_t)ssidLen;                                    // update SSID length byte
    memcpy(&tmpPacket[38 + ssidLen], &beaconPacket[70], wpa2 ? 39 : 13); // copy second half of packet into buffer

    const bool success = sendPacket(tmpPacket, tmpPacketSize, ch, false);
